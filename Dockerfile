...
ENV MOZJPEG_VER="3.2"
RUN cd ~ && \
  wget -O mozjpeg.tar.gz "https://github.com/mozilla/mozjpeg/releases/download/v${MOZJPEG_VER}/mozjpeg-${MOZJPEG_VER}-release-source.tar.gz" && \
	tar xf mozjpeg.tar.gz && \
	cd mozjpeg && \
	./configure CFLAGS="-O2 -march=native" --without-simd --prefix=/opt/mozjpeg && \
  make -j$(nproc) > /dev/null && \
  make install
